...
#define T_SMC_FIFO_IOCTL 5  /* fifo ioctl */
#define T_SMC_SET_ISPR 0x10 /* set interrupt set-pending register address */

#define NT_SMC_SWITCH 0		/* switch to T */
#define	 NT_SMC_SWITCH_OK	 0	/* no error */
#define	 NT_SMC_SWITCH_ERR	1	/* error, return to NT */

#define NT_SMC_FIFO_IOCTL 3	/* fifo ioctl*/
#define NT_SMC_STEAL_GET 0x10   /* get stolen time */


#ifndef __ASSEMBLY__

struct csrvisor_fifo_ioctl;
...
{
	register unsigned long r0 asm("r0") = T_SMC_SET_ISPR;
...
static inline void csrvisor_fifo_ioctl(struct csrvisor_fifo_ioctl *ioctl)
{
	register unsigned long r0 asm("r0") = T_SMC_FIFO_IOCTL;
	register unsigned long r1 asm("r1") = (unsigned long)ioctl;
...
static inline void csrvisor_fifo_ioctl(struct csrvisor_fifo_ioctl *ioctl)
{
	register unsigned long r0 asm("r0") = NT_SMC_FIFO_IOCTL;
	register unsigned long r1 asm("r1") = (unsigned long)ioctl;
