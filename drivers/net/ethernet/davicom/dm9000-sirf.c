...
dm9000_probe(struct platform_device *pdev)
{
...
#else
	db->data_req = db->addr_req;
	db->io_data = db->io_addr;
#endif
	...
	for (i = 0; i < 8; i++) {
		...
		if ((id_val == DM9000_ID) || (id_val == DM9000_ID_SIRF))
			break;
...
static struct platform_device sirf_device_eth = {
	.name   = "dm9000-sirf",
	.id     = -1,
	.num_resources  = ARRAY_SIZE(sirf_dm9000_resource),
	.resource       = sirf_dm9000_resource,
	.dev    = {
		.platform_data  = &sirf_dm9000_pdata,
	},
};
...
dm9000_cleanup(void)
{
	...
	platform_device_unregister(&sirf_device_eth);
